...
if(mePlayer != noone) {
	...
	for(var i = 0; i < ds_list_size(clients); i++) {
		...
		draw_text(posX,48 + (posY*i), currentClient.username + ":{Ping: " + string(currentClient.ping) + ",Pos:{X=" + string(currentClient.x) + ",Y=" + string(currentClient.y) + "},TargetPos:{X=" + string(currentClient.targetX) + ",Y="+string(currentClient.targetY)+"}}"); 	
	}
}